...
# Script-scoped state is kept here so helper functions can write warnings,
# resolve the final evidence path, and know whether a transcript is active.
$script:InvokedScriptPath = $PSCommandPath
$script:EvidenceRoot = $null
$script:Warnings = @()
$script:TranscriptStarted = $false

# Writes consistent console status lines for live incident handling.
# This gives Service Desk staff a simple timeline of what the script is doing.
...
# Captures non-fatal collection problems into an in-memory warning list.
# The script stays read-only and keeps collecting other evidence instead of stopping.
...
# Wraps each evidence collection stage so one failed artifact does not end the run.
# This is important in incident response because permissions and local state vary widely.
...
# Resolves the directory where the script is running from so the default output path
# can be placed beside the script even in PowerShell 5.1.
...
# Calculates the default deployment window anchor. The default assumption is that
# the suspected rollout started on the previous Friday at noon local time.
...
# Creates directories only when not in DryRun mode.
# In DryRun mode it reports what would happen without touching the file system.
...
# Builds the timestamped evidence folder name for this workstation.
# This keeps repeated collections separate and preserves chain-of-evidence context.
...
# Serializes structured objects to JSON for machine-readable evidence review.
function Save-Json {
    param(
        [string]$FileName,
        $Data,
...
# Serializes tabular evidence to CSV for quick analyst filtering and comparison.
function Save-Csv {
    param(
        [string]$FileName,
        [object[]]$Data
...
# Writes plain-text artifacts for command-line outputs that are not naturally structured.
function Save-Text {
    param(
        [string]$FileName,
        [string[]]$Lines
...
# Converts raw byte values to rounded gigabytes for easier human review.
...
# Attempts to normalize common uninstall registry date formats into DateTime values.
# This makes it possible to compare install times to the Friday deployment window.
...
# Reads a property defensively from loosely shaped objects.
# Many Windows data sources omit fields, and strict mode would otherwise terminate the run.
...
# Checks whether a string contains one of the deployment-related name patterns.
# The same helper is reused across software, tasks, services, processes, and events.
...
# Collects baseline device identity details.
# This anchors the evidence set to a specific machine, OS build, boot session, and user context.
...
# Reads installed application inventory from standard uninstall registry locations.
# This avoids Win32_Product and other invasive techniques that can trigger repairs or slowdowns.
...
# Filters the installed software list down to applications installed on or after the
# suspected deployment window so analysts can quickly spot recent changes.
...
# Narrows installed software to entries likely associated with the DMS deployment.
# This helps determine whether the target application or related components are present.
...
# Enumerates startup commands that could delay logon or launch deployment components
# during user sign-in.
...
# Enumerates scheduled tasks and flattens their actions/triggers into readable strings.
# Deployment packages often register tasks that run at logon or on a schedule.
...
# Captures currently running processes, their resource footprint, and likely ownership.
# This supports investigation of poor workstation performance and active app components.
...
# Takes a point-in-time performance snapshot for CPU, memory, and fixed disks.
# This does not trend over time, but it quickly shows whether the workstation is under load.
...
# Filters local services down to entries associated with the suspected deployment.
# Services can explain startup drag, authentication side effects, or agent health issues.
...
# Normalizes a raw Windows event into a simpler shape for CSV and JSON output.
# The message is rendered at collection time so the evidence is readable offline later.
...
# Queries event logs by filter and converts them to a stable output shape.
# Empty logs are treated as empty datasets rather than fatal errors.
...
# Combines separate event sets into a single time-sorted sequence.
# This is used to merge security and system-side authentication evidence.
...
# Collects the event evidence most relevant to this hypothesis:
# system errors, application errors, logon/authentication events, Group Policy,
# user profile processing, and application-specific matches.
...
# Runs gpresult for both computer and user scope.
# This provides additional policy-processing evidence even when event logs are incomplete.
...
# Collects user profile, desktop-path, redirection, and temporary-profile indicators.
# These data points are central to the missing shortcut and slow logon hypotheses.
...
# Inventories desktop shortcuts in both the user and public desktop locations.
# It also resolves shortcut targets when possible and flags items changed since deployment.
...
# Captures local network configuration and domain-connectivity indicators.
# This supports login-failure analysis and helps separate deployment issues from network or trust problems.
...
# Builds a cross-artifact view of items most likely tied to the deployment window.
# This gives responders a fast shortlist instead of forcing them to correlate manually.
...
# Apply safe defaults after parameter binding so the script remains compatible with PowerShell 5.1.
...
# Create the evidence folder structure before starting collection.
Initialize-EvidenceFolder

# Emit the starting run context so the operator can see exactly how this run was scoped.
...
try {
    # Start a transcript for non-DryRun executions so the operator actions and script output
    # are preserved alongside the structured evidence files.
    if (-not $DryRun) {
    ...
    # Collect workstation identity and operating system facts first.
    # These details are needed to tie all other evidence back to a specific endpoint state.
    ...
    # Capture the full installed software inventory, then save both analyst-friendly CSV
    # and machine-friendly JSON representations.
    ...
    # Highlight recent installations that overlap the suspected Friday deployment window.
    ...
    # Reduce the software inventory to items that match the deployment name patterns.
    ...
    # Inventory startup items that may be contributing to logon delay or post-logon slowdown.
    ...
    # Inventory scheduled tasks because software deployments often create background launchers,
    # update tasks, or logon-triggered jobs.
    ...
    # Capture the live process list to show what is actually running on the endpoint right now.
    ...
    # Take a current performance snapshot so the incident team can compare subjective slowness
    # to observed CPU, memory, and disk pressure.
    ...
    # Capture only services associated with the suspected deployment rather than dumping every service.
    ...
    # Pull recent system, application, logon, Group Policy, user profile, and app-specific events.
    # These are some of the highest-value artifacts for proving or disproving deployment impact.
    ...
    # Run gpresult to preserve textual evidence of effective policy processing state.
    ...
    # Collect profile and desktop-path evidence before evaluating shortcut placement.
    ...
    # Inventory shortcut files from both desktop scopes and preserve their timestamps and targets.
    ...
    # Capture network and domain-trust indicators to help rule in or rule out authentication,
    # DNS, or secure-channel issues unrelated to the app deployment.
    ...
    # Build a final deployment-focused artifact list that correlates recent installs,
    # changed shortcuts, and matched application events into one timeline-friendly file.
    ...
    # Summarize the collection at a glance so an engineer can see the dataset scope,
    # notable signals, and warning count without opening every artifact file first.
    ...
    # Persist the summary and any non-fatal collection warnings for later review.
    ...
    # Emit a final completion message so the live operator knows the run finished.
    Write-Status -Message ('Collection complete. Warnings={0}. EvidenceFolder={1}' -f @($script:Warnings).Count, $script:EvidenceRoot)
}
finally {
    # Stop the transcript if one was started, even if a later stage threw an exception.
